...
const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  return <div className="border rounded-md overflow-hidden bg-white shadow-sm">
      <div className="border-b bg-muted/40 px-4 py-2 flex items-center justify-between bg-gray-950">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" className="bg-debug-purple hover:bg-debug-purple/90" onClick={handleRun}>
      ...
      {showHint && <div className="bg-debug-purple-light border-b border-debug-purple/20 p-4">
          <div className="flex items-start justify-between">
            ...
            <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => setShowHint(false)}>
              <X className="h-4 w-4" />
            </Button>
          </div>
      ...
      <div className="relative">
        <textarea ref={editorRef} value={code} onChange={e => setCode(e.target.value)} spellCheck={false} className="code-editor w-full h-96 p-4 resize-none outline-none font-mono bg-gray-950" />
        <div className="absolute top-0 left-0 p-4 pointer-events-none">
          {code.split('\n').map((_, index) => <div key={index} className="code-line-number">{index + 1}</div>)}
        </div>
      </div>
